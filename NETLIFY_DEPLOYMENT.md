...
- [x] `public/robots.txt` and `public/sitemap.xml` updated to `https://cosmovertex.edu.bd`
...
2. Click **Add a domain** and enter `cosmovertex.edu.bd` (or your domain).
3. Choose one of the two DNS options:
   ...
   - **External DNS**:
     - Apex domain (`cosmovertex.edu.bd`): Add an `A` record pointing to Netlify's load balancer IP `75.2.60.5`.
     - Subdomain (`www.cosmovertex.edu.bd`): Add a `CNAME` record pointing to your Netlify site name (`your-site.netlify.app`).
